...
export class UI {
	...
	async run(config, manifest) {
		try {
			...
			if (!fs.existsSync(templateDir)) {
				throw new Error(
					`Templates directory does not exist: ${templateDir}. UI generation requires templates to be present in the specified templates directory.`,
				);
			}
			...
			if (templateFiles.length === 0) {
				throw new Error(
					`Templates directory is empty: ${templateDir}. UI generation requires template files to be present for proper generation.`,
				);
			}
			...
			console.log(
				`Using provided manifest with ${commandManifests.length} command manifests`,
			);
			...
			const commandForms = commandManifests.map((cmd) =>
				this.generateCommandForm(cmd, initialState, templateDir),
			);
			...
			const html = await this.generateSeparateFiles(
				globalManifest,
				commandManifests,
				commandForms,
				cssContent,
	...
	generateCommandForm(commandManifest, state, templateDir) {
		...
		if (commandManifest.examples && commandManifest.examples.length > 0) {
			...
			const examplesTemplatePath = path.join(
				templateDir,
				'command-examples.html',
			);
			let examplesTemplate = fs.readFileSync(examplesTemplatePath, 'utf8');
			examplesTemplate = examplesTemplate.replace(
				'{{EXAMPLE_LIST}}',
				examplesList,
			);
		...
		formTemplate = formTemplate
			...
			.replace(
				/{{COMMAND_DESCRIPTION}}/g,
				this.escapeHtml(commandManifest.description || ''),
			)
			.replace('{{PARAMETER_FIELDS}}', parameterFieldsHtml)
			.replace(
				/{{FORM_SUBMIT_BUTTON}}/g,
				this.escapeHtml(this.constants.DEFAULTS.FORM_SUBMIT_BUTTON),
			)
	...
	async generateSeparateFiles(
		...
		const clientSideJS = this.generateClientSideJavaScript(
			globalManifest,
			commandManifests,
		);
		...
		baseTemplate = baseTemplate
			.replace(
				'{{HTML_TITLE}}',
				this.escapeHtml(
					globalManifest.name || this.constants.DEFAULTS.HTML_TITLE,
				),
			)
			.replace(
				'{{PAGE_HEADER}}',
				this.escapeHtml(
					globalManifest.description || this.constants.DEFAULTS.PAGE_HEADER,
				),
			)
			.replace(
				'{{COMMAND_SELECTOR_LABEL}}',
				this.escapeHtml(this.constants.DEFAULTS.COMMAND_SELECTOR_LABEL),
			);
