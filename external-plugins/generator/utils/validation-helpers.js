...
export class ValidationHelpers {
	...
	static validateParam(value, paramManifest, state = {}) {
		...
		if (this.isNumericType(paramManifest.type)) {
			if (
				paramManifest.min !== undefined &&
				Number(value) < paramManifest.min
			) {
				return {
					isValid: false,
			...
			if (
				paramManifest.max !== undefined &&
				Number(value) > paramManifest.max
			) {
				return {
					isValid: false,
		...
		if (paramManifest.type === 'string') {
			if (
				paramManifest.minLength !== undefined &&
				String(value).length < paramManifest.minLength
			) {
				return {
					isValid: false,
			...
			if (
				paramManifest.maxLength !== undefined &&
				String(value).length > paramManifest.maxLength
			) {
				return {
					isValid: false,
