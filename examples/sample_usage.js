...
5. **Train and generate:**
   \`\`\`
   markov> build_dict("shakespeare.txt", order=3)
...
|---------|-------------|----------|
| \`build_dict(file, order)\` | Train model from text file | \`build_dict("corpus.txt", order=3)\` |
...
\`\`\`javascript
const customFilter = (text, options) => {
...
\`\`\`javascript
class CustomGenerator extends TextGenerator {
...
- Memory usage scales with corpus size and model order
- Order 2-3 models work well for most texts
- Higher orders (4+) require larger training corpora
- Models with 10k+ states may take longer to save/load
