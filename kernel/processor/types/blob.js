...
export default {
  ...
  validate(value, spec) {
    ...
    // Size constraints
    if (spec.constraints?.maxSize && blob.size && blob.size > spec.constraints.maxSize) {
      throw new RangeError(`Blob exceeds maximum size: ${blob.size} > ${spec.constraints.maxSize}`);
    }
    ...
    if (spec.constraints?.allowedTypes && blob.mimeType) {
      if (!spec.constraints.allowedTypes.includes(blob.mimeType)) {
        throw new TypeError(`Blob type '${blob.mimeType}' not allowed. Allowed: ${spec.constraints.allowedTypes.join(', ')}`);
      }
    }
    ...
    if (spec.constraints?.allowedExtensions && blob.name) {
      ...
      if (ext && !spec.constraints.allowedExtensions.includes(ext)) {
        throw new TypeError(`Blob extension '.${ext}' not allowed. Allowed: ${spec.constraints.allowedExtensions.map(e => '.' + e).join(', ')}`);
      }
    }
    ...
    if (value && typeof value === 'object') {
      ...
      if (normalized.data && !Buffer.isBuffer(normalized.data)) {
        if (Array.isArray(normalized.data)) {
          ...
          normalized.data = Buffer.from(normalized.data, normalized.encoding || 'utf8');
        } else if (normalized.data && typeof normalized.data === 'object' && normalized.data.type === 'Buffer' && Array.isArray(normalized.data.data)) {
