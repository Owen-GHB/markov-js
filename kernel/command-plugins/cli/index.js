 ...
 */
function getCliInstance(commandRoot, contextFilePath) {
	if (!cliInstance) {
		cliInstance = new CLI(commandRoot, contextFilePath);
	}
	return cliInstance;
}
 ...
 */
export async function run(commandRoot, contextFilePath, args) {
	const cli = getCliInstance(commandRoot, contextFilePath);
